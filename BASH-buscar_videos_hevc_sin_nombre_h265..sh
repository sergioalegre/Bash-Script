#@Sergio Alegre - Mayo 2023
#Este script busca en el path dado como parametro de manera recursiva, archivos de videos que h265 (hevc) y cuyo nombre no tenga el texto h265
...
function path(){
    ...
    for f in "${lista[@]}"; do #recorro los ficheros uno a uno
        ...
        if [[ "${codec}" = 'hevc' ]] #si codec = h265
        then
            if [[ $f == *h265* ]] # si en el nombre tiene h265
            then
                ...
                echo -ne "${endColour}"
                lista_corregir+=("\n$f") #lista de videos hevc que no tienen h265 en su nombre
            fi
        fi
    done

    echo -e "\n${yellowColour} Videos hevc que no tienen h265 en su nombre en $path_escanear\n${lista_corregir[@]}${endColour}"
}
